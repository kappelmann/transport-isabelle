...
definition "half_galois_prop_left \<equiv>
  \<forall>x y. in_codom (\<le>\<^bsub>R\<^esub>) y \<longrightarrow> x \<le>\<^bsub>L\<^esub> r y \<longrightarrow> l x \<le>\<^bsub>R\<^esub> y"

notation galois_prop.half_galois_prop_left (infix "\<^sub>h\<unlhd>" 50)

lemma half_galois_prop_leftI [intro]:
  assumes "\<And>x y. in_codom (\<le>\<^bsub>R\<^esub>) y \<Longrightarrow> x \<le>\<^bsub>L\<^esub> r y \<Longrightarrow> l x \<le>\<^bsub>R\<^esub> y"
...
lemma half_galois_prop_rightI [intro]:
  assumes "\<And>x y. in_dom (\<le>\<^bsub>L\<^esub>) x \<Longrightarrow> l x \<le>\<^bsub>R\<^esub> y \<Longrightarrow> x \<le>\<^bsub>L\<^esub> r y"
...
lemma rel_unit_if_left_rel_if_mono_wrt_relI:
  ...
  and "in_dom (\<le>\<^bsub>L\<^esub>) x \<Longrightarrow> l x \<le>\<^bsub>R\<^esub> l x' \<Longrightarrow> x \<le>\<^bsub>L\<^esub> \<eta> x'"
...
corollary rel_unit_if_left_rel_if_half_galois_prop_right_if_mono_wrt_rel:
  ...
  shows "x \<le>\<^bsub>L\<^esub> \<eta> x'"
  using assms
  by (auto intro: rel_unit_if_left_rel_if_mono_wrt_relI half_galois_prop_rightD)
...
corollary rel_unit_if_reflexive_on_if_half_galois_prop_right_if_mono_wrt_rel:
  ...
  using assms by (blast
    intro: rel_unit_if_left_rel_if_half_galois_prop_right_if_mono_wrt_rel)
...
corollary inflationary_on_unit_if_reflexive_on_if_half_galois_prop_rightI:
  ...
  using assms by (intro inflationary_onI) (fastforce intro:
    rel_unit_if_reflexive_on_if_half_galois_prop_right_if_mono_wrt_rel)

interpretation flip : galois_prop R L r l .

lemma right_rel_if_right_left_rel_right_if_deflationary_onI:
  ...
  and "r y \<le>\<^bsub>L\<^esub> r y'"
  and "in_dom (\<le>\<^bsub>R\<^esub>) y"
  and "P y'"
...
corollary counit_rel_if_right_rel_if_mono_wrt_relI:
  assumes "((\<le>\<^bsub>R\<^esub>) \<Rrightarrow>\<^sub>m (\<le>\<^bsub>L\<^esub>)) r"
  and "in_codom (\<le>\<^bsub>R\<^esub>) y' \<Longrightarrow> r y \<le>\<^bsub>L\<^esub> r y' \<Longrightarrow> \<epsilon> y \<le>\<^bsub>R\<^esub> y'"
...
corollary left_rel_if_left_right_rel_left_if_inflationary_onI:
  assumes "((\<le>\<^bsub>L\<^esub>) \<Rrightarrow>\<^sub>m (\<le>\<^bsub>R\<^esub>)) l"
  ...
  using assms by (intro flip_inv.right_rel_if_right_left_rel_right_if_deflationary_onI
    [simplified rel_inv_iff_rel])
