...
proof (rule flip.left_relI)
  ...
  with mono_r1 half_galois_prop_left1 have [iff]: "\<epsilon>\<^sub>1 x1' \<le>\<^bsub>R1\<^esub> x1'"
    by (intro g1.counit_rel_if_right_rel_if_half_galois_prop_left_if_mono_wrt_rel)
...
lemma left_rel_right_if_left_right_relI:
  ...
  and "g \<le>\<^bsub>L\<^esub> g"
  and "l g \<le>\<^bsub>R\<^esub> f"
  shows "g \<le>\<^bsub>L\<^esub> r f"
  using assms
...
lemma half_galois_prop_left_left_rightI:
  ...
  unfolding left_rel_eq_Refl_Rel right_rel_eq_Refl_Rel using assms
  by (intro
    half_galois_prop_leftI[unfolded left_rel_eq_Refl_Rel right_rel_eq_Refl_Rel]
    Refl_Rel_app_leftI[where ?f=l]
    t.left_right_rel_if_left_rel_rightI)
  auto
...
lemma half_galois_prop_right_left_rightI:
  ...
  unfolding left_rel_eq_Refl_Rel right_rel_eq_Refl_Rel using assms
  by (intro
    half_galois_prop_rightI[unfolded left_rel_eq_Refl_Rel right_rel_eq_Refl_Rel]
    Refl_Rel_app_rightI[where ?f=r]
    t.left_rel_right_if_left_right_relI)
  auto
...
lemma galois_prop_left_rightI':
  assumes "(t.L \<Rrightarrow>\<^sub>m t.R) l" and "(t.R \<Rrightarrow>\<^sub>m t.L) r"
  and galois_conn1: "((\<le>\<^bsub>L1\<^esub>) \<stileturn> (\<le>\<^bsub>R1\<^esub>)) l1 r1"
  and "reflexive_on (in_codom (\<le>\<^bsub>L1\<^esub>)) (\<le>\<^bsub>L1\<^esub>)"
  and "reflexive_on (in_dom (\<le>\<^bsub>R1\<^esub>)) (\<le>\<^bsub>R1\<^esub>)"
  and "\<And>x'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow>
    ((\<le>\<^bsub>L2 (r1 x') (r1 x')\<^esub>) \<^sub>h\<unlhd> (\<le>\<^bsub>R2 (\<epsilon>\<^sub>1 x') x'\<^esub>)) (l2\<^bsub> x' (r1 x')\<^esub>) (r2\<^bsub>(r1 x') x'\<^esub>)"
  ...
  and "\<And>x'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow> (\<le>\<^bsub>R2 (\<epsilon>\<^sub>1 x') x'\<^esub>) \<le> (\<le>\<^bsub>R2 x' x'\<^esub>)"
  and "\<And>x1' x2'. x1' \<le>\<^bsub>R1\<^esub> x2' \<Longrightarrow> (\<le>\<^bsub>R2 x1' x1'\<^esub>) \<le> (\<le>\<^bsub>R2 x1' x2'\<^esub>)"
  and mono_l2_2: "\<And>x. x \<le>\<^bsub>L1\<^esub> x \<Longrightarrow>
  ...
  and mono_r2_2: "\<And>x'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow>
  ...
  shows "((\<le>\<^bsub>L\<^esub>) \<unlhd> (\<le>\<^bsub>R\<^esub>)) l r"
proof -
  from mono_l2_2 have "\<And>x y. x \<le>\<^bsub>L1\<^esub> x \<Longrightarrow> in_dom (\<le>\<^bsub>L2 x (\<eta>\<^sub>1 x)\<^esub>) y \<Longrightarrow>
    (\<le>\<^bsub>R2 (l1 x) (l1 x)\<^esub>) (l2\<^bsub>(l1 x) (\<eta>\<^sub>1 x)\<^esub> y) \<le> (\<le>\<^bsub>R2 (l1 x) (l1 x)\<^esub>) (l2\<^bsub>(l1 x) x\<^esub> y)"
    using galois_conn1 trans_R2 by blast
  moreover from mono_r2_2  have "\<And>x' y'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow> in_codom (\<le>\<^bsub>R2 (\<epsilon>\<^sub>1 x') x'\<^esub>) y' \<Longrightarrow>
    (\<ge>\<^bsub>L2 (r1 x') (r1 x')\<^esub>) (r2\<^bsub>(r1 x') (\<epsilon>\<^sub>1 x')\<^esub> y') \<le> (\<ge>\<^bsub>L2 (r1 x') (r1 x')\<^esub>) (r2\<^bsub>(r1 x') x'\<^esub> y')"
    using galois_conn1 trans_L2 by blast
  ultimately show ?thesis using assms by (intro galois_prop_left_rightI) auto
qed

lemma galois_prop_left_right_half_galois_prop_assms_if_galois_propI:
  assumes "((\<le>\<^bsub>L1\<^esub>) \<Rrightarrow>\<^sub>m (\<le>\<^bsub>R1\<^esub>)) l1" and "((\<le>\<^bsub>R1\<^esub>) \<Rrightarrow>\<^sub>m (\<le>\<^bsub>L1\<^esub>)) r1" and "((\<le>\<^bsub>L1\<^esub>) \<unlhd>\<^sub>h (\<le>\<^bsub>R1\<^esub>)) l1 r1"
  ...
  shows "\<And>x'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow>
    ((\<le>\<^bsub>L2 (r1 x') (r1 x')\<^esub>) \<^sub>h\<unlhd> (\<le>\<^bsub>R2 (\<epsilon>\<^sub>1 x') x'\<^esub>)) (l2\<^bsub> x' (r1 x')\<^esub>) (r2\<^bsub>(r1 x') x'\<^esub>)"
  and "\<And>x. x \<le>\<^bsub>L1\<^esub> x \<Longrightarrow> ((\<le>\<^bsub>L2 x (\<eta>\<^sub>1 x)\<^esub>) \<unlhd>\<^sub>h (\<le>\<^bsub>R2 (l1 x) (l1 x)\<^esub>)) (l2\<^bsub>(l1 x) x\<^esub>) (r2\<^bsub>x (l1 x)\<^esub>)"
proof -
  show "\<And>x'. x' \<le>\<^bsub>R1\<^esub> x' \<Longrightarrow>
    ((\<le>\<^bsub>L2 (r1 x') (r1 x')\<^esub>) \<^sub>h\<unlhd> (\<le>\<^bsub>R2 (\<epsilon>\<^sub>1 x') x'\<^esub>)) (l2\<^bsub> x' (r1 x')\<^esub>) (r2\<^bsub>(r1 x') x'\<^esub>)"
    using assms by fastforce
  show "((\<le>\<^bsub>L2 x (\<eta>\<^sub>1 x)\<^esub>) \<unlhd>\<^sub>h (\<le>\<^bsub>R2 (l1 x) (l1 x)\<^esub>)) (l2\<^bsub>(l1 x) x\<^esub>) (r2\<^bsub>x (l1 x)\<^esub>)" if "x \<le>\<^bsub>L1\<^esub> x" for x
  proof -
    from \<open>x \<le>\<^bsub>L1\<^esub> x\<close> have "x \<^bsub>L1\<^esub>\<lessapprox> l1 x" using assms by blast
    with galois_prop2 show ?thesis by fastforce
  qed
qed

corollary galois_prop_left_right_if_galois_propI:
  assumes "(t.L \<Rrightarrow>\<^sub>m t.R) l" and "(t.R \<Rrightarrow>\<^sub>m t.L) r"
  and "((\<le>\<^bsub>L1\<^esub>) \<stileturn> (\<le>\<^bsub>R1\<^esub>)) l1 r1"
  and "reflexive_on (in_codom (\<le>\<^bsub>L1\<^esub>)) (\<le>\<^bsub>L1\<^esub>)"
  and "reflexive_on (in_dom (\<le>\<^bsub>R1\<^esub>)) (\<le>\<^bsub>R1\<^esub>)"
  ...
  using assms by (intro galois_prop_left_rightI'
    galois_prop_left_right_half_galois_prop_assms_if_galois_propI)
  auto

lemma galois_prop_left_right_le_assms_left_if_galois_propI:
...
proof -
  ...
  with galois_conn1 refl_L1 have "x1 \<le>\<^bsub>L1\<^esub> x1" "x2 \<le>\<^bsub>L1\<^esub> \<eta>\<^sub>1 x2"
    by (blast intro:
      g1.rel_unit_if_left_rel_if_half_galois_prop_right_if_mono_wrt_rel)+
...
lemma galois_prop_left_right_le_assms_right_if_galois_propI:
  assumes galois_conn1: "((\<le>\<^bsub>L1\<^esub>) \<stileturn> (\<le>\<^bsub>R1\<^esub>)) l1 r1"
...
proof -
  ...
  with galois_conn1 refl_R1 have "x2' \<le>\<^bsub>R1\<^esub> x2'" "\<epsilon>\<^sub>1 x1' \<le>\<^bsub>R1\<^esub> x1'"
    by (blast intro:
      g1.counit_rel_if_right_rel_if_half_galois_prop_left_if_mono_wrt_rel)+
...
corollary galois_prop_left_right_if_mono_if_galois_propI:
  ...
  and "\<And>x1' x2'. x1' \<le>\<^bsub>R1\<^esub> x2' \<Longrightarrow> transitive (\<le>\<^bsub>R2 x1' x2'\<^esub>)"
  shows "((\<le>\<^bsub>L\<^esub>) \<unlhd> (\<le>\<^bsub>R\<^esub>)) l r"
  using assms by (intro galois_prop_left_right_if_galois_propI
...
text \<open>Note that we could further rewrite @{thm "galois_prop_left_rightI'"},
as we will do later for Galois connections, by applying
@{thm "t.mono_wrt_rel_leftI"} and @{thm "t.mono_wrt_rel_rightI"} to the
first premises. However, this is not really helpful here.
Moreover, the resulting theorem will not result in a
useful lemma for the flipped instance of @{locale transport_Dep_Fun_Rel_rel}
since @{thm "t.mono_wrt_rel_leftI"} and @{thm "t.mono_wrt_rel_rightI"} are not
flipped dual but only flipped-inversed dual.\<close>
